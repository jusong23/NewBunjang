...
// MARK: - [GET] 8.2 브랜드 검색하기 /brands
class get_8_2_SearchBrands{
    func getSearchBrands(accessToken:String, searchWord:String, onCompleted: @escaping (Result<SearchBrands,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/brands?search="
        ...
        AF.request(url,
                   ...
                   parameters: param, // 서버로 보낼 parameter를 담는 것(POST)
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(SearchBrands.self, from: data)
