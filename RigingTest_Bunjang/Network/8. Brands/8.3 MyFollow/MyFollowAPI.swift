...
// MARK: - [GET] 8.3 내가 팔로우한 브랜드 가져오기
class get_8_2_MyFollow{
    func getMyFollow(accessToken:String, userIdx:String, onCompleted: @escaping (Result<MyFollow,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/brands/\(userIdx)"
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(MyFollow.self, from: data)
