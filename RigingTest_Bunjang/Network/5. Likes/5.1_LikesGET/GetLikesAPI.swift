...
// MARK: - [GET] 5.1 찜 목록 조회 /likes
class get_5_1_likes{
    func getLikes(accessToken:String, userIdx:String, onCompleted: @escaping (Result<LikesGet,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/likes/\(userIdx)"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(LikesGet.self, from: data)
