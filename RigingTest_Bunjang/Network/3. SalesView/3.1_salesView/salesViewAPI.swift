...
// MARK: - [GET] 3.1 최근 본 상품 조회 /salesView
class get_3_1_salesView{
    func getSalesView(accessToken:String, userIdx:String, onCompleted: @escaping (Result<SalesView,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/sales-views/\(userIdx)"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(SalesView.self, from: data)
