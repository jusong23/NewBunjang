...
// MARK: - [GET] 10.3 채팅방 전부 가져오기 /cht-romms
class get_10_3_chatRoom{
    func getChathRoom(accessToken:String, onCompleted: @escaping (Result<ChatRooms,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/chat-rooms"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(ChatRooms.self, from: data)
