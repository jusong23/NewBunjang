...
class get_12_1_order_indirect{
    func getOrderIndirect(accessToken:String, itemIdx:Int, onCompleted: @escaping (Result<OrderIndirect,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/orders/indirect/\(itemIdx)/0"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(OrderIndirect.self, from: data)
