...
class get_12_4_purchase{
    func getPurchase(accessToken:String, onCompleted: @escaping (Result<Purchase,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/orders/purchase/"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(Purchase.self, from: data)
