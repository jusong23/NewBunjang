...
// MARK: - [GET] 7.2 검색 페이지 조회하기 /search
class get_7_2_search{
    func getAddress(accessToken:String, userIdx:String, onCompleted: @escaping (Result<Search_7_2,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/searches/\(userIdx)"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(Search_7_2.self, from: data)
