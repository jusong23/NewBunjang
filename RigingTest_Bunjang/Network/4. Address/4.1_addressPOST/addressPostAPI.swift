...
// MARK: - [POST] 4.1 배송지 등록 /addresses/create
class post_4_1 {
    ...
    func post_4_1_(accessToken:String, userName:String, address:String, detailAddress:String, phoneNumber:String, isBaseAddress:Int, userIdx:Int ) {
            ...
            var addressStruct = AddressPOSTRequestBody(userName: userName, address: address, detailAddress: detailAddress, phoneNumber: phoneNumber, isBaseAddress: isBaseAddress, userIdx: userIdx)
            
            guard let jsonData = try? JSONEncoder().encode(addressStruct) else {
