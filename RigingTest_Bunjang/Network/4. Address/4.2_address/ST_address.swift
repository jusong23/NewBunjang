...
// MARK: - [GET] 4.2 배송지 정보 가져오기 /address
class get_4_2_address{
    func getAddress(accessToken:String, onCompleted: @escaping (Result<Address_GET,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/addresses"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(Address_GET.self, from: data)
