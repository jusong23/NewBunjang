...
// MARK: - [GET] 13.2 결제 수단 조회 /patments/:userIdx
class get_13_2_payments{
    func getPayments(accessToken:String,userIdx:String, onCompleted: @escaping (Result<PaymentUserIDGETResponse,Error>)-> Void) {
        let url = "https://prod.jinsoo.shop/payments/\(userIdx)"
        AF.request(url,
        ...
        .responseData(completionHandler: { response in // 응답데이터를 받을수 있는 메소드를 Chaning
            ...
            case let .success(data): // 요청 O
                do { // 요청 O 응답 O
                    ...
                    let result = try decoder.decode(PaymentUserIDGETResponse.self, from: data)
